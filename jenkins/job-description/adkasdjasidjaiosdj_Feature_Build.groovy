...
def job_config = [
    job: [
        name: "adkasdjasidjaiosdj_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'adkasdjasidjaiosdj_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/adkasdjasidjaiosdj_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.Adkasdjasidjaiosdj.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "kkanto", "adkasdjasidjaiosdj", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("adkasdjasidjaiosdj_Feature_Build");
...
job.with {
    ...
    authenticationToken('9PeTfREjxiTfpZH3vhxe2c7i73JYLAWb')

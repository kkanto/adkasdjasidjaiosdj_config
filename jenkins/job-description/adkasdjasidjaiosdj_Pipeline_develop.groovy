...
def job_config = [
        job: [
                name: "adkasdjasidjaiosdj_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "adkasdjasidjaiosdj", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("adkasdjasidjaiosdj_Pipeline_develop");
